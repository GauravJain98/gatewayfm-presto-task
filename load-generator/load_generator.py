...
# Prometheus metrics
transaction_counter = Counter('eth_transactions_total', 'Total transactions sent', ['status'])
rpc_requests_counter = Counter('eth_rpc_requests_total', 'Total RPC requests', ['method', 'status'])
transaction_latency = Histogram('eth_transaction_latency_seconds', 'Transaction latency in seconds')
...
class EthereumLoadGenerator:
    ...
    async def check_connection(self):
        ...
        try:
            ...
            balance_result = await self.rpc_call("eth_getBalance", [self.account.address, "latest"])
    ...
    async def rpc_call(self, method: str, params: list = None) -> Any:
        ...
        try:
            async with self.session.post(self.config.geth_url, json=payload) as response:
    ...
    async def send_transaction(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get nonce
            nonce_result = await self.rpc_call("eth_getTransactionCount", [self.account.address, "pending"])
            ...
            # Send transaction
            tx_hash = await self.rpc_call("eth_sendRawTransaction", [signed_tx.rawTransaction.hex()])
    ...
    async def transaction_worker(self):
        ...
        logger.info(f"Starting transaction worker with target TPS: {self.config.target_tps}")
    ...
    async def metrics_updater(self):
        ...
        while True:
            ...
            if time_diff > 0:
                ...
                success_tx = transaction_counter.labels(status="success")._value._value
                error_tx = transaction_counter.labels(status="error")._value._value
                total_tx = success_tx + error_tx
                if total_tx > 0:
                    failure_rate.set(error_tx / total_tx)
